...
module.exports = {
  ...
  knight: (ctx, knightList) => new Promise((res, rej) => {
    function recursive(List) {
      List.map((el) => {
        const keyTrans = ctx.i18n.__(`key.${el.key}`)
        if (el.dataRequired && !el.data) rej({ code: 400, msg: ctx.i18n.__('error.attribute', keyTrans) })
        if (typeof el.data === 'object' && el.dataRequired && Object.keys(el.data).length === 0) rej({ code: 400, msg: ctx.i18n.__('error.empty-object', keyTrans) })
        ...
        if (typeof el.data === 'object') {
          ...
          el.entries.map((entry) => {
            if (entry.entryRequired && el.data[entry.key] === undefined) rej({ code: 400, msg: ctx.i18n.__('error.attribute', ctx.i18n.__(`key.${entry.key}`)) })
